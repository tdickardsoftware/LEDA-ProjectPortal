...
export default function RostersContent() {
	...
	const handleRemoveDivision = useCallback(
		(division: string) => {
			...
			const teamsToRemove = Object.values(
				updatedDivisions[division].subdivisions
			).flatMap((subdivision) =>
			...
			setSelectedTeams(
				selectedTeams.filter(
					(teamId) => !teamsToRemove.includes(teamId)
				)
			);
			setHasChanges(true); // Explicitly set hasChanges to true
		},
	...
	const updateSubdivisionNames = useCallback(
		(division: string) => {
			...
			const newSubdivisions: {
				[
					key: string
				]: (typeof updatedSubdivisions)[keyof typeof updatedSubdivisions];
			} = {};
			let index = 1;

			Object.keys(updatedSubdivisions).forEach((subdivision) => {
				newSubdivisions[`Subdivision ${index}`] =
					updatedSubdivisions[subdivision];
				index++;
			});
	...
	const handleRemoveSubdivision = useCallback(
		(division: string, subdivision: string) => {
			const updatedDivisions = { ...divisionsData };
			const teamsToRemove = Object.values(
				updatedDivisions[division].subdivisions[subdivision]
	...
	const handleRemoveTeam = useCallback(
		async (
			...
			console.log(
				"Before removal:",
				JSON.stringify(divisionsData) === JSON.stringify(initialData)
			);
			const updatedDivisions = { ...divisionsData };
			...
			console.log(
				"After removal:",
				JSON.stringify(updatedDivisions) === JSON.stringify(initialData)
			);
			setHasChanges(true); // Explicitly set hasChanges to true
		},
	...
	const handleDeleteRoster = useCallback(async () => {
		try {
			...
			setUpdate(false);
			setDivisionsData({});
			setHasChanges(false); // Reset hasChanges after successful deletion
		} catch (error) {
