...
export default function ManageGlobalAdjustments({
    ...
    const [globalAdjustments, setGlobalAdjustments] = React.useState<GlobalAdjustmentMap>({});
    const [selectedAdjustmentId, setSelectedAdjustmentId] = React.useState<string | null>(null);
    const [editDialogOpen, setEditDialogOpen] = React.useState(false); // State for edit dialog
    const [editingAdjustment, setEditingAdjustment] = React.useState<AdjustmentItem | null>(null);
    ...
    React.useEffect(() => {
        ...
        Object.keys(payoutsData).forEach(division => {
            Object.keys(payoutsData[division]).forEach(subdivision => {
                Object.keys(payoutsData[division][subdivision]).forEach(teamId => {
                    const teamAdjustments = payoutsData[division][subdivision][teamId].adjustments;
                    
                    if (teamAdjustments) {
                        Object.entries(teamAdjustments).forEach(([adjustmentId, adjustment]) => {
                            // Only process global adjustments
                            if (adjustment.global) {
                                // If this global adjustment hasn't been seen yet, initialize it
                                if (!extractedAdjustments[adjustmentId]) {
                                ...
                                extractedAdjustments[adjustmentId].appliedTeamsCount++;
                            }
                        });
                    }
                });
    ...
    const confirmDelete = (adjustmentId: string) => {
        if (window.confirm("This will remove the adjustment from all teams. This action cannot be undone.")) {
    ...
    const handleAdjustmentUpdate = (adjustmentId: string, updatedAdjustment: AdjustmentItem) => {
        const updatedGlobalAdjustments = { ...globalAdjustments };
        
        // Ensure amount has exactly two decimal places
        const formattedAmount = Number(Math.abs(updatedAdjustment.adjustmentAmount).toFixed(2));
        updatedAdjustment.adjustmentAmount = updatedAdjustment.credit ? formattedAmount : -formattedAmount;
        
        if (updatedGlobalAdjustments[adjustmentId]) {
            updatedGlobalAdjustments[adjustmentId].adjustment = updatedAdjustment;
        }
                ...
                <Table>
                    ...
                    <TableBody>
                        {Object.keys(globalAdjustments).length > 0 ? (
                            Object.entries(globalAdjustments).map(([adjustmentId, { adjustment, appliedTeamsCount }]) => (
                                <TableRow
                                    key={adjustmentId}
                                    className={highlightedAdjustmentId === adjustmentId ? "bg-yellow-100" : ""}
                                >
                                    <TableCell className="font-medium">
                                        ${Math.abs(adjustment.adjustmentAmount).toFixed(2)}
                                    </TableCell>
                                    <TableCell>
                                        <span className={adjustment.credit ? "text-green-600" : "text-red-600"}>
                                            {adjustment.credit ? "Credit" : "Debit"}
                                        </span>
                                    </TableCell>
                                    <TableCell>{adjustment.notes}</TableCell>
                                    <TableCell>{appliedTeamsCount} teams</TableCell>
                                    <TableCell className="flex space-x-2">
                                        <Button 
                                            variant="ghost" 
                                            size="sm"
                                            className="text-blue-600 hover:text-blue-700 hover:bg-transparent p-1 h-auto"
                                            onClick={() => handleEdit(adjustmentId)}
                                        >
                                            <Pencil size={18} />
                                        </Button>
                                        <span 
                                            className="text-red-600 hover:text-red-700 hover:bg-transparent p-1 h-auto cursor-pointer"
                                            onClick={() => confirmDelete(adjustmentId)}
                                        >
                                            <X size={18} />
                                        </span>
                                    </TableCell>
            ...
            {editingAdjustment && (
                <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
                    ...
                    >
                        ...
                        <p id="edit-adjustment-description" className="text-sm text-gray-500">
