...
export default function SeasonAddForm({ onClose, onRefresh }: { onClose: () => void, onRefresh: () => void })  {
    ...
    async function onSubmit(values: z.infer<typeof seasonFormSchema>) {
        const formattedDates = JSON.parse(dates || '[]').reduce((acc: any, date: string, index: number) => {
            acc[`Date${index + 1}`] = new Date(date).toLocaleDateString('en-US');
            return acc;
        }, {});
                ...
                <div className='flex space-x-4'>
                    <div className={formContainerStyle}>
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <Label className='whitespace-nowrap pr-2' htmlFor="isCurrentSeasonCheckbox">
                                        Current Season?
                                    </Label>
                                    <FormControl>
                                        <Checkbox
                        ...
                        />
                        <div className='flex space-x-4'>
                            <InputDefault control={form.control} name="seasonCode" label="Season Code *" customClass={inputWidth}/>
                            <InputDefault control={form.control} name="fiscalYear" label="Fiscal Year *" customClass={inputWidth}/>
                        </div>
                        <div className='flex items-start space-x-2'>
                            <Label className='whitespace-nowrap' htmlFor='generateDates'>
                                Automatically Generate Dates? - Manual Entry Coming Later
                            </Label>
                            <Checkbox
                                checked={autoGenerateDates}
                                onCheckedChange={(checked: boolean) => setAutoGenerateDates(checked)}
                                className={checkboxWidth}
                                id='generateDates'
                                disabled
                            />
                        ...
                        {autoGenerateDates && (
                            <>
                                <Label className={`whitespace-nowrap`} htmlFor='howManyWeeks'> How Many Weeks? </Label>
                                <Input 
                                    id='howManyWeeks' 
                                    type="number" 
                                ...
                                />
                                <Label className='whitespace-nowrap' htmlFor='initialDate'> Initial Date </Label>
                                <Input 
                                    id='initialDate' 
                                    type="date" 
                                ...
                                <div className='mt-2 max-w-[65vw] overflow-x-auto'>
                                    <Label className='whitespace-nowrap text-gray-500'>Generated Dates</Label>
                                    <Separator className='my-2' />
                                    <table className='max-w-[65vw] divide-y divide-gray-200 overflow-auto'>
                                        <thead className='bg-gray-50'>
                                            <tr>
                                                {dates && Array.isArray(JSON.parse(dates)) && JSON.parse(dates).map((date: string, index: number) => (
