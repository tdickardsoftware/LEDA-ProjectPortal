...
function getTableChunks(data: TrailsMembershipHistory[], rowsPerCol: number, cols: number): TrailsMembershipHistory[][][] {
...
export const TrailsMembershipHistoryReport: React.FC<TrailsMembershipHistoryReportProps> = ({
  data,
    ...
    <Document>
      {pages.map((tablesOnPage, pageIdx) => (
        <Page key={pageIdx} size="A4" style={styles.page} wrap>
          <ReportsHeader title="Trails Membership History" reportDate={reportDate} />
          {Array.from({ length: TABLES_PER_PAGE }).map((_, slotIdx) => {
            const tableData = tablesOnPage[slotIdx];
            
            // If no table data for this slot, render empty space to maintain layout
            if (!tableData) {
              return <View key={slotIdx} style={styles.tableGap} />;
            }

            // Determine how many columns are actually needed for this table
            const actualColumns = tableData.filter(col => col.length > 0);
            ...
            return (
              <View key={slotIdx}>
                  ...
                  <View style={styles.tableHeader}>
                    {actualColumns.map((_, colIdx) => (
                      <React.Fragment key={colIdx}>
                        <Text style={[styles.cell, styles.ledaIdColumn, styles.tableHeaderText]}>LEDA#</Text>
                        <Text style={[
                          styles.cell,
                          styles.nameColumn,
                          styles.tableHeaderText,
                          ...(colIdx === actualColumns.length - 1 ? [styles.lastCell] : [])
                        ]}>Name</Text>
                      </React.Fragment>
                    ))}
                  </View>
                ...
                {slotIdx < TABLES_PER_PAGE - 1 && <View style={styles.tableGap} />}
              </View>
            );
          })}
          <ReportsFooter />
