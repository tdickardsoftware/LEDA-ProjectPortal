...
import { MentionPlayerHistory, PlayerMemberInfo, TopDarterTotals, WeeklyTopDarterScores } from "@/lib/definitions";
...
export default function PlayerTDPHistoryContent( {
    ...
    const getPlayerMentions = useCallback(async (
        seasonCode: string, 
        teamId: number,
        weekNum?: number
    ) => {
    ...
    const handleGameSelect = (seasonCode: string, teamName: string, weekNum: string) => {
        ...
        if (teamId) {
            ...
            getPlayerMentions(seasonCode, teamId, weekNumInt).then(mentions => {                
                setMentionData({
                    seasonCode,
                    teamId,
                    weekNum: weekNumInt,
    ...
    const calculateBasePoints = (weekData: WeekDataType | null | undefined) => {
        if (!weekData || !mentionData.mentions || mentionData.mentions.length === 0) {
            return weekData?.totalPoints || 0;
        }
        
        // Sum all mention points for this week
        const mentionPointsTotal = mentionData.mentions.reduce((sum, mention) => {
    ...
    const basePoints = selectedGameData ? calculateBasePoints(selectedGameData) : 0;
                        ...
                        <div className="p-6 border rounded-lg shadow-sm bg-white">
                            ...
                            <div className="mt-4">
                                <div className="flex justify-between items-center p-4 border rounded-md bg-gray-50">
                                    <div>
                                        <h4 className="text-lg font-medium">{selectedTeam} Season Summary</h4>
                                        <p className="text-gray-600">Season: {selectedSeason}</p>
                                        {/* Calculate base points (total points minus mention points) */}
                                        <p className="text-gray-600 mt-1">
                                            Base Points: {teamTotalPoints - mentionData.mentions.reduce((sum, mention) => sum + (mention.mentionPoints * (mention.count || 1)), 0)} pts
                                        </p>
                                        <p className="text-gray-600 mt-2">
                                            Mention Points: {mentionData.mentions.reduce((sum, mention) => sum + (mention.mentionPoints * (mention.count || 1)), 0)} pts
                                        </p>
                                    </div>
                            ...
                            <div className="mt-6">
                                <h3 className="text-lg font-semibold mb-3">Mentions for this season</h3>
                                {mentionData.loading ? (
                                    ...
                                    <div className="space-y-3">
                                        {mentionData.mentions
                                            .sort((a, b) => {
                                                ...
                                                return (b.creationDate ? new Date(b.creationDate).getTime() : 0) - 
                                            ...
                                            .map((mention, index) => (
                                                <div key={index} className="p-3 border rounded-md bg-gray-50">
                                                    <div className="flex justify-between items-start">
                                                        <div>
                                                            <h4 className="font-medium">{mention.mentionDesc}</h4>
                                                            <p className="text-sm text-gray-600">
                                                                Week {mention.weekNum} • Code: {mention.mentionCode}
                                                                {mention.count > 1 && ` • Count: ${mention.count}`}
                                                            </p>
                                                            {mention.notes && (
                                                                <p className="text-sm mt-1 italic text-gray-500">{mention.notes}</p>
                                                            )}
                                                            <p className="text-xs text-gray-500 mt-1">{formatDate(typeof mention.creationDate === "string" ? mention.creationDate : mention.creationDate?.toISOString())}</p>
                                                        </div>
                                                        <span className={`font-bold px-2 py-1 rounded-md ${
                                                            mention.mentionPoints > 0 
                                                                ? 'bg-green-100 text-green-700' 
                                                                : mention.mentionPoints < 0 
                                                                    ? 'bg-red-100 text-red-700' 
                                                                    : 'bg-gray-100 text-gray-700'
                                                        }`}>
                                                            {mention.mentionPoints > 0 ? '+' : ''}{mention.mentionPoints * (mention.count || 1)} pts
                                                        </span>
                                                    </div>
                                                </div>
                                            ))
                                    ...
                                    <p className="text-gray-500">No mentions found for this season.</p>
                                )}
                            </div>
                            
                            <p className="text-gray-600 mt-6">
                                Select a game from the sidebar to view detailed weekly information.
                            </p>
                        </div>
                    ) : selectedGameData ? (
                        <div className="p-6 border rounded-lg shadow-sm bg-white">
                            ...
                            <div className="mt-4">
                                <div className="flex justify-between items-center p-4 border rounded-md bg-gray-50">
                                    <div>
                                        ...
                                        <p className="text-gray-600">Season: {selectedSeason}</p>
                                        <p className="text-gray-600 mt-1">Base Points: {basePoints} pts</p>
                                        <p className="text-gray-600 mt-2">
                                            Mention Points: {mentionData.mentions.reduce((sum, mention) => sum + (mention.mentionPoints * (mention.count || 1)), 0)} pts
                                        </p>
                                    </div>
                            ...
                            <div className="mt-6">
                                <h3 className="text-lg font-semibold mb-3">Week {selectedGame} Mentions</h3>
                                {mentionData.loading ? (
                                    <p className="text-gray-500">Loading mentions...</p>
                                ) : mentionData.mentions && mentionData.mentions.length > 0 ? (
                                    <div className="space-y-3">
                                        {mentionData.mentions.map((mention, index) => (
                                            <div key={index} className="p-3 border rounded-md bg-gray-50">
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <h4 className="font-medium">{mention.mentionDesc}</h4>
                                    ...
                                    <p className="text-gray-500">No mentions found for this week.</p>
                                )}
                            </div>
                        </div>
                    ) : (
