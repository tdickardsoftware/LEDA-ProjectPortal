		...
		<div className="container mx-auto p-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
				<Card className="shadow-lg">
					...
					<CardContent>
						<p className="text-lg">
							Fiscal Year: <span className="font-medium">{seasonData.fiscalYear}</span>
						</p>
						<p className="text-lg">
							Current Season? : <span className="font-medium">{seasonData.isCurrentSeason ? "Yes" : "No"}</span>
						</p>
					</CardContent>
				</Card>
				<Card className="shadow-lg">
					...
					<CardContent>
						{Object.entries(seasonData.dates).map(([key, value]) => (
							<p key={key} className="text-lg">
								{key.replace(/(\D+)(\d+)/, "$1 $2")}: <span className="font-medium">{value}</span>
							</p>
						))}
					</CardContent>
				</Card>
