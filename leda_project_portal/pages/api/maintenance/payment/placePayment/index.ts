...
export default async function handler(
    ...
    if  (req.method === "GET") {
        if (req.query.ledaId) {
            try {
            // Execute the database query to fetch payment history and fullName for a specific ledaId
            ...
            try {
            // Execute the database query to fetch payment history and fullName for all records
            const result = await query<PaymentHistory & { fullName: string }>(
                `SELECT h."paymentNbr", h."ledaId", h."type", h."paymentType", h."amount", h."seasonCode", h."comp", h."notes", h."paidOff", h."date",
                    p."name" AS "fullName"
                 FROM maint.leda_maint_place_payment_history h
