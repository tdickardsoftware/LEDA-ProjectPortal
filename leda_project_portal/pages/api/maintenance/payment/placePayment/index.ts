...
async function getLastPaidBarSeasonInfo(ledaId: string, excludeSeasonCode?: string) {
    const queryStr = `
...
export default async function handler(
    ...
    if  (req.method === "GET") {
        ...
        try {
            ...
            if ((data.paymentNbr === null || data.paymentNbr === undefined) && !unpaidPartPayments) {
                ...
                if ((data.type === "Part" || data.type === "Memb") && data.paidOff === true) {
                    ...
                    const paymentInfo = getLastPaymentInfoResult.rows[0]?.lastBarFeePayment || "";

                    if (paymentInfo.includes("UNPAID")) {
                        await updateLastBarFeePayment(data.ledaId.toString(), data.seasonCode);
                    } else {
                ...
                if (getPartPaymentPaidStatusResult.rows.length > 0) {
                    ...
                    const pastPaid = await getLastPaidBarSeasonInfo(data.ledaId.toString(), data.seasonCode);
                    if (pastPaid) {
                        await updateLastBarFeePayment(data.ledaId.toString(), pastPaid.seasonCode);
                    } else {
        ...
        try {
            ...
            if ((data.type === "Part" || data.type === "Memb") && data.paidOff === true) {
                const paidStatus = await getLastPaidBarSeasonInfo(data.ledaId.toString());
                if (paidStatus) {
                    await updateLastBarFeePayment(data.ledaId.toString(), paidStatus.seasonCode);
                } else {
                    await setBarFeeUnpaid(data.ledaId.toString());
                }
            }
