...
async function getFiscalYear(seasonCode: string) {
    const result = await query(`SELECT "fiscalYear" FROM maint.leda_maint_seasons WHERE "seasonCode" = $1;`, [seasonCode]);
    return result.rows[0]?.fiscalYear;
}

async function updateLastMembershipFeePayment(ledaId: string, seasonCode: string) {
...
async function getLastPaidSeasonInfo(ledaId: string, excludeSeasonCode?: string) {
    const queryStr = `
...
async function markAllUnpaidPartsPaid(type: string, seasonCode: string, ledaId: string) {
...
export default async function handler(
    ...
    if (req.method === "GET") {
        ...
        try {
            ...
            const result = await query<PaymentHistory & { fullName: string; fiscalYear: string }>(sql, params);
    ...
    if (req.method === "POST") {
        try {
            ...
            if ((data.paymentNbr === null || data.paymentNbr === undefined) && !unpaidPartPayments) {
                ...
                if ((data.type === "Part" || data.type === "Memb") && data.paidOff === true) {
                    ...
                    if (paymentInfo.includes("UNPAID")) {
                        await updateLastMembershipFeePayment(data.ledaId.toString(), data.seasonCode);
                    } else {
                        ...
                        const [lastSeason, currentSeason] = await Promise.all([
                            query(`SELECT dates->>'Date1' as date1 FROM maint.leda_maint_seasons WHERE "seasonCode" = $1;`, [lastSeasonCode]),
                            query(`SELECT dates->>'Date1' as date1 FROM maint.leda_maint_seasons WHERE "seasonCode" = $1;`, [data.seasonCode])
                        ]);
                        const lastSeasonCodeDate1 = lastSeason.rows[0]?.date1;
                        const currentSeasonCodeDate1 = currentSeason.rows[0]?.date1;
            ...
            } else if ((data.paymentNbr !== null && data.paymentNbr !== undefined) && data.type === "Part" && data.paidOff === false) {
                ...
                if (getPartPaymentPaidStatusResult.rows.length > 0) {
                    ...
                    const pastPaid = await getLastPaidSeasonInfo(data.ledaId.toString(), data.seasonCode);
                    if (pastPaid) {
                        await updateLastMembershipFeePayment(data.ledaId.toString(), pastPaid.seasonCode);
                    } else {
                ...
                await markAllUnpaidPartsPaid(data.type, data.seasonCode, data.ledaId.toString());
                await updateLastMembershipFeePayment(data.ledaId.toString(), data.seasonCode);
    ...
    if (req.method === "DELETE") {
        try {
            ...
            if ((data.type === "Part" || data.type === "Memb") && data.paidOff === true) {
                const paidStatus = await getLastPaidSeasonInfo(data.ledaId.toString());
                if (paidStatus) {
                    await updateLastMembershipFeePayment(data.ledaId.toString(), paidStatus.seasonCode);
                } else {
                    await setMembershipFeeUnpaid(data.ledaId.toString());
                }
            }
