...
const log = createRouteLogger("/api/maintenance/payment/playerPayment/datatable");
...
export default async function handler(
	...
	if (req.method === "GET") {
		log.info({ method: "GET", query: req.query }, "Fetch player payment datatable request");
		try {
			...
			const sqlColumnMapping = new Map<string, string>([
				...
				["ledaId", 'h."ledaId"'],
				["fullName", "CONCAT(COALESCE(p.\"firstName\", ''), ' ', COALESCE(p.\"middleInitial\", ''), ' ', COALESCE(p.\"lastName\", ''))"],
				["amount", 'h."amount"'],
				["date", 'h."date"'],
				["type", 'h."type"'],
			...
			log.info({ page, pageSize, totalRecords }, "Fetched player payment datatable");
			res.status(200).json({
