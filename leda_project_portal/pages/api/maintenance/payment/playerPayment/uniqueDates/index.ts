        ...
        if (req.query.ledaId) {
            try {
                ...
                const result = await query(
                    'SELECT DISTINCT "paymentDate" FROM maint.leda_maint_player_payment_history WHERE "ledaId" = $1 ORDER BY "paymentDate";',
            ...
            try {
                ...
                const result = await query(
                    'SELECT DISTINCT "paymentDate" FROM maint.leda_maint_player_payment_history ORDER BY "paymentDate";'
                );
                // Respond with the query result
                res.status(200).json(result.rows);
            } catch (error) {
