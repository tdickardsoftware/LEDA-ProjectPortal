/**
 * Paginated server-side datatable endpoint for team payment history.
 *
 * GET - Returns a paginated, optionally filtered and searched slice of
 *       leda_maint_team_payment_history joined with team and season info.
...
const log = createRouteLogger("/api/maintenance/payment/teamPayment/datatable");
...
export default async function handler(
	...
	if (req.method === "GET") {
		log.info({ method: "GET", query: req.query }, "Fetch team payment datatable request");
		try {
			...
			log.info({ page, pageSize, totalRecords }, "Fetched team payment datatable");
			res.status(200).json({
			...
			log.error({ err: error }, "Failed to fetch team payment history");
			res.status(500).json({
