...
const log = createRouteLogger("/api/maintenance/payment/teamPayment/viewData");
...
export default async function handler(
	...
	if (!session) return;
	if (req.method === "GET") {
		log.info({ method: "GET", query: req.query }, "Fetch team payment view data request");
		if (req.query.seasonCode) {
			try {
				...
				log.info({ seasonCode: req.query.seasonCode, count: result.rows.length }, "Fetched team paid status by season");
				res.status(200).json(result.rows);
			} catch (error) {
				log.error({ err: error }, "Failed to fetch team paid status by season");
				res.status(500).json({
			...
			try {
				...
				log.info({ ledaId: req.query.ledaId }, "Fetched team paid status by ledaId");
				res.status(200).json(result.rows[0]);
			} catch (error) {
				log.error({ err: error }, "Failed to fetch team paid status by ledaId");
				res.status(500).json({
					message: "Failed to fetch roster information",
					error,
				});
