...
async function getLastPaidTeamSeasonInfo(ledaId: string, excludeSeasonCode?: string) {
    const queryStr = `
...
async function markAllUnpaidTeamPartsPaid(type: string, seasonCode: string, ledaId: string) {
...
export default async function handler(
    ...
    if  (req.method === "GET") {
        ...
        try {
            ...
            if ((data.paymentNbr === null || data.paymentNbr === undefined) && !unpaidPartPayments) {
                ...
                if ((data.type === "Part" || data.type === "Memb") && data.paidOff === true) {
                    ...
                    const paymentInfo = getLastPaymentInfoResult.rows[0]?.lastTeamFeePayment || "";

                    if (paymentInfo.includes("UNPAID")) {
                        await updateLastTeamFeePayment(data.ledaId.toString(), data.seasonCode);
                    } else {
                ...
                if (getPartPaymentPaidStatusResult.rows.length > 0) {
                    ...
                    const pastPaid = await getLastPaidTeamSeasonInfo(data.ledaId.toString(), data.seasonCode);
                    if (pastPaid) {
                        await updateLastTeamFeePayment(data.ledaId.toString(), pastPaid.seasonCode);
                    } else {
                ...
                await markAllUnpaidTeamPartsPaid(data.type, data.seasonCode, data.ledaId.toString());
                await updateLastTeamFeePayment(data.ledaId.toString(), data.seasonCode);
        ...
        try {
            ...
            if ((data.type === "Part" || data.type === "Memb") && data.paidOff === true) {
                const paidStatus = await getLastPaidTeamSeasonInfo(data.ledaId.toString());
                if (paidStatus) {
                    await updateLastTeamFeePayment(data.ledaId.toString(), paidStatus.seasonCode);
                } else {
                    await setTeamFeeUnpaid(data.ledaId.toString());
                }
            }
