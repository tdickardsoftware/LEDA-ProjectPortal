...
export default async function handler(
    ...
    if  (req.method === "GET") {
        if (req.query.ledaId) {
            try {
                ...
                const result = await query<PaymentHistory>(
                    'SELECT "paymentNbr", "ledaId", "type", "paymentType", "amount", "seasonCode", "paymentDate" FROM maint.leda_maint_team_payment_history WHERE "ledaId" = $1 ORDER BY "paymentNbr";',
            ...
            try {
                ...
                const result = await query<PaymentHistory>(
                    'SELECT "paymentNbr", "ledaId", "type", "paymentType", "amount", "seasonCode", "paymentDate" FROM maint.leda_maint_team_payment_history ORDER BY "paymentNbr";'
                );
                // Respond with the query result
                res.status(200).json(result.rows);
            } catch (error) {
        ...
        try {
            ...
            const query = 'INSERT INTO maint.leda_maint_team_payment_history("paymentNbr", "ledaId", "type", "paymentType", "amount", "seasonCode", "paymentDate") VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT ("paymentNbr") DO UPDATE SET "ledaId" = $2, "type" = $3, "paymentType" = $4, "amount" = $5, "seasonCode" = $6, "paymentDate" = $7;'
