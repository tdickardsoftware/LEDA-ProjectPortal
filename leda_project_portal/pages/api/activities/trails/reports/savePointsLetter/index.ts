...
const log = createRouteLogger("/api/activities/trails/reports/savePointsLetter");
...
export default async function handler(
	...
	// Handle GET requests
	if (req.method === "GET") {
		log.info({ method: "GET", query: req.query }, "Fetch trails save-points letter data");
		try {
			...
			const result = await query<TrailsSavePointsLetter>(
				`SELECT v."ledaId",
						...
						v."addressFirstLine", v."addressSecondLine", v."lastTrailsDate", v."totalpoints"
				 FROM public.leda_reports_trails_save_points v
