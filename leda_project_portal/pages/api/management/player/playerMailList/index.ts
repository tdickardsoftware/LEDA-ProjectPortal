        ...
        if (req.body.alreadySelected !== undefined) {
            try {
                ...
                if (alreadySelected.length === 0) {
                    // No exclusions, get all players
                    const result = await query<MailingList>(`SELECT "ledaId", concat(COALESCE("lastName", ''::text), ', ', COALESCE("firstName", ''::text), ' ', COALESCE("middleInitial", ''::character varying)) AS "name", concat(COALESCE("addressOne", ''::text), ' ', COALESCE("addressTwo", ''::text)) AS "addressLineOne", concat(COALESCE(city, ''::text), ', ', COALESCE(state, ''::text), ' ', COALESCE(zip, ''::text::character varying)) AS "addressLineTwo", 'PLAYER' as "type" FROM leda_player_info`)
                    ...
                    const result = await query<MailingList>(
                        `SELECT "ledaId", concat(COALESCE("lastName", ''::text), ', ', COALESCE("firstName", ''::text), ' ', COALESCE("middleInitial", ''::character varying)) AS "name", concat(COALESCE("addressOne", ''::text), ' ', COALESCE("addressTwo", ''::text)) AS "addressLineOne", concat(COALESCE(city, ''::text), ', ', COALESCE(state, ''::text), ' ', COALESCE(zip, ''::text::character varying)) AS "addressLineTwo", 'PLAYER' as "type" FROM leda_player_info WHERE "ledaId" NOT IN (${placeholders})`,
                        alreadySelected
                    );
                    return res.status(200).json(result.rows);
                }
        ...
        if (req.query.availableOnly) {
            ...
            try {
                const result = await query<MailingList>(`
                    SELECT 
                        p."ledaId", 
                        concat(COALESCE(p."lastName", ''::text), ', ', COALESCE(p."firstName", ''::text), ' ', COALESCE(p."middleInitial", ''::character varying)) AS "name", 
            ...
            try {
                const result = await query<MailingList>(`SELECT "ledaId", "name", concat(COALESCE("addressOne", ''::text), ' ', COALESCE("addressTwo", ''::text)) AS "addressLineOne", concat(COALESCE(city, ''::text), ', ', COALESCE(state, ''::text), ' ', COALESCE(zip, ''::text::character varying)) AS "addressLineTwo", "PLAYER" as "type" FROM leda_player_info`)
