...
const log = createRouteLogger("/api/management/player/playerNotPaid");
...
export default async function handler(
	...
	// Handle GET requests
	if (req.method === "GET") {
		log.info({ method: "GET", query: req.query }, "Fetch players not paid request");
		try {
            if (req.query.seasonCode) {
			// Execute the database query to fetch season code information
			const result = await query<PlayerNotPaid>(
				'SELECT "seasonCode", "ledaId", "fullName" FROM public.leda_reports_league_play_player_not_paid WHERE "seasonCode" = $1',
