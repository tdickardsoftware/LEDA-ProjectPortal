...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'GET' && req.body.type === 'player') {
        try {
    ...
    } else if (req.method === 'GET' && req.body.type === 'placeOwner') {
        try {
            const result = await query<Player>(`SELECT p."ledaId", CONCAT(COALESCE(p."firstName", ''), ' ', COALESCE(p."middleInitial", ''), ' ', COALESCE(p."lastName", '')) as "fullName", p."lastName", p."firstName", p."middleInitial", m."memberType" FROM public.leda_player_info p 
