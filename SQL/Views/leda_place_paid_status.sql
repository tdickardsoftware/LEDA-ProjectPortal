-- View: public.leda_place_paid_status

-- DROP VIEW public.leda_place_paid_status;

CREATE OR REPLACE VIEW public.leda_place_paid_status
 AS
    ...
    COALESCE(
        CASE
            WHEN (EXISTS ( SELECT 1
               FROM maint.leda_maint_place_payment_history p
              WHERE p."seasonCode" = r."seasonCode" AND p."ledaId" = ((teams.value ->> 'placeId'::text)::bigint) AND (p.type = 'Bar'::text OR p.type = 'Part'::text AND p."paidOff" = true))) THEN 'PAID'::text
            WHEN (EXISTS ( SELECT 1
               FROM maint.leda_maint_place_payment_history p
              WHERE p."seasonCode" = r."seasonCode" AND p."ledaId" = ((teams.value ->> 'placeId'::text)::bigint) AND p.type = 'Part'::text AND (p."paidOff" = false OR p."paidOff" IS NULL))) THEN 'PART'::text
            ELSE 'UNPAID'::text
